...
impl GameAsset {
    ...
    fn load_from_json_game_asset(json_game_asset :JSONGameAsset, window: &mut PistonWindow) -> GameAsset {

        let frames: Vec<AssetFrame> = json_game_asset.frames.iter().map(|json_frame| {
            return AssetFrame {
                src_rect: [
                    json_frame.frame.x,
                    json_frame.frame.y,
        ...
        let texture = match Texture::from_path(&mut window.create_texture_context(), 
        format!("../assets/tests/{}", json_game_asset.meta.image.clone()),
        Flip::None,
        &TextureSettings::new()) {
            Ok(texture) => texture,
            Err(error) => {
                println!("Fail to load texture ({} PNG): {}", json_game_asset.meta.image, error);
                std::process::exit(2);
            }
        };
...
pub fn load_assets(window: &mut PistonWindow) -> HashMap<EntityAssets, GameAsset> {
    let assets_list = vec![
        (EntityAssets::Character(Animations::Idle), "../assets/tests/character_animation-Idle.json"),
        (EntityAssets::Character(Animations::Run), "../assets/tests/character_animation-Run.json")
    ];

    return assets_list.iter().map(|asset_data| {
...
pub fn load_hard_drown_assets(window: &mut PistonWindow) -> HashMap<HardTexture, G2dTexture> {
    ...
    let loaded_assets : HashMap<HardTexture, G2dTexture> = assets.iter().map(|path| {
