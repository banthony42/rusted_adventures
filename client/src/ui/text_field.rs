...
pub struct TextField<T> where T: ContentFormat {
    font_size: u32,
    initial_rect: [u32; 4],
    rect: [u32; 4],
    content: Vec<T>,
...
impl<T> TextField<T> where T: ContentFormat {
    ...
    pub fn render(&self, evnt : &Event, window: &mut PistonWindow, font: &mut Font) {
        ...
        let _ = self.content.iter().map(|content| {
            let (text_color, text) = content.content_format();
           
            line_height += font.text_height_for_max_width(text.as_str(),
            self.font_size,
            self.rect[2] as f64 - 8.0) as f64;

            let msg_position = [
                self.rect[0] as f64 + 1.0,
                self.rect[1] as f64 + self.rect[3] as f64 + 10.0 - line_height + (self.scroll * self.font_size as f64)
            ];

            font.render_text_max_width(text.as_str(),
            self.font_size,
            evnt,
            window,
            text_color,
