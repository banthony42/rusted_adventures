...
<p align="center">
  ...
  <a href="https://www.dofus-retro.com/en/mmorpg/discover" target="_blank" rel="noopener noreferrer">
    <img src="https://img.shields.io/badge/humbly_inspired_by_dofus-83af1f?style=for-the-badge&logo=egghead&logoColor=white"
         alt="Humbly inspired by Dofus"></a>
</p>
...
block-beta
  columns 5
  Client_1 Empty["..."] Client_n space:2
  space:3 CLI space
  block:group1:3
  DockerLabel["Docker Host"]
  block
    Server["Server"] space
  end
  block
...
| Component | Description | Read more |
|-----------|-------------|---------------|
| 🎮 Client | Game client application. | [README](./client/) |
| 🖥️ Server | gRPC services and world engine. | [README](./server/) |
| 🔧 CLI | Administration and testing tools. | [README](./rpg-cli/) |
| 📚 Common Library | Shared code, database access, gRPC code generation, and utilities. | [README](./common/) |
| ⛁ Database | Persistent storage for accounts, players, and entities. | [README](./common/src/database/) |
...
- Copy the `.env.default` file to `.env` and fill in the required environment variables.

##### Build and run server and database:
```sh
docker compose build
docker compose up
```
##### Build the CLI and client:
```sh
cargo build --bin rpg-cli --bin rpg-client
```

##### Create your account:
```sh
POSTGRES_HOST=localhost ./target/debug/rpg-cli account <user> <password>
```
...
Validate the server through its public gRPC API using the generated client and a real database.

- Located in [server/tests](./server/tests/) folder.
